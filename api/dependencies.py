...
from providers.glm import ZAI_BASE_URL, ZAIProvider
...
def _create_provider_for_type(provider_type: str, settings: Settings) -> BaseProvider:
    ...
    if provider_type == "glm":
        ...
        config = ProviderConfig(
            ...
            base_url=settings.zai_base_url or ZAI_BASE_URL,
            rate_limit=settings.provider_rate_limit,
            rate_window=settings.provider_rate_window,
            max_concurrency=settings.provider_max_concurrency,
            http_read_timeout=settings.http_read_timeout,
    ...
    logger.error(
        "Unknown provider_type: '{}'. Supported: 'nvidia_nim', 'open_router', 'lmstudio', 'llamacpp', 'glm'",
        provider_type,
    )
    raise ValueError(
